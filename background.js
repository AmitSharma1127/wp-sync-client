...
// Default variable to check if user is a host or not
...
function create_twitch_endpoint() {
	...
	let openid_url = `https://id.twitch.tv/oauth2/authorize?client_id=${CLIENT_ID}&redirect_uri=${REDIRECT_URI}&response_type=${RESPONSE_TYPE}&scope=${SCOPE}&state=${STATE}&nonce=${nonce}&claims=${CLAIMS}`;
	...
	if (request.check_user_signedIn) {
		...
		chrome.storage.sync.get(["wpsync_display_name"], function(res) {
			if (!request.is_private) {
				...
				createWindowPopup(
					`https://www.` + request.service + `.com/`,
					1200,
					800,
					10,
					0,
					"js/overlay_popup.js",
					"css/overlay_popup.css"
				);
				// Open chat window when lobby is public
				createWindowPopup(
					"https://www.twitch.tv/popout/" + party_id + "/chat?darkpopout='",
					350,
					800,
					10,
					1200,
					"js/wp_twitch_chat.js",
					"none"
				);
			} else {
		...
		chrome.windows.create(
			{
				...
				type: "popup",
				width: Math.round(
					request.create_window.right - request.create_window.left
				),
				height: Math.round(
					request.create_window.bottom - request.create_window.top
				),
		...
		fetch(DOMAIN_URL + "view/join/" + request.joinBtnClick)
			.then(res => res.json())
			.then(res => {
