chrome.storage.sync.get(["DOMAIN_URL", "wp_is_host_viewer"], function(result) {
	...
	if (
		...
		window.location.pathname.split("/")[1] == "view"
	) {
	...
	function joinBtnClick() {
		...
		is_private = document
			.getElementById("join")
			.getAttribute("data-aria-isPrivate");
		platform = document
			.getElementById("join")
			.getAttribute("data-aria-platform");
	...
	if (
		window.location.host == "www.twitch.tv" &&
		window.location.pathname.split("/")[1] !== "popout"
	) {
		chrome.storage.sync.get("wp_is_host_viewer", function(e) {
			...
			if (wp_is_host_viewer) {
				window.onload = function() {
					setTimeout(() => {
						document.querySelector("#live-page-chat > div").style.height =
							"65vh";
						document.querySelector("#live-page-chat > div").style.top = "28vh";
						document.getElementsByClassName(
							"InjectLayout-sc-1i43xsx-0 kTZbIF right-column__toggle-visibility toggle-visibility__right-column toggle-visibility__right-column--expanded"
						)[0].style.top =
							"31vh";
						...
						document.querySelector(
							"#root > div > div.Layout-sc-1xcs6mc-0.kBprba > div > main > div.root-scrollable.scrollable-area.scrollable-area--suppress-scroll-x > div.simplebar-scroll-content > div > div > div.InjectLayout-sc-1i43xsx-0.persistent-player > div"
						).style.height =
							"46vh";
						document
							.querySelector(
